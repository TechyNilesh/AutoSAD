...
from pysad_rust import RobustRandomCutForest
...
def run_rrcf(dataset_name, run_count=None, seed=42, progress_interval=1000, output_dir="benchmark_results", verbose=True):
    """Run Robust Random Cut Forest algorithm on a dataset."""
    ...
    try:
        ...
        # Initialize model
        model = RobustRandomCutForest(**rrcf_params)
            ...
            if processed_instances % progress_interval == 0:
                ...
                if verbose:
                    if run_count is not None:
                        print(f"RRCF on {dataset_name} (Run {run_count}): Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)
                    else:
                        print(f"RRCF on {dataset_name}: Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)                
        ...
        results = {
            ...
            "model": "RRCF", # Robust Random Cut Forest
            "auc": final_auroc,
        ...
        print(f"RRCF completed on {dataset_name}")
        ...
        print(f"Error running RRCF on {dataset_name}: {str(e)}")
        return None

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Robust Random Cut Forest on a dataset')
    parser.add_argument('--dataset', type=str, required=True, help='name of the dataset to run RRCF on')
