...
from pysad_rust import IForestASD
...
def run_iforestasd(dataset_name, run_count=None, seed=42, progress_interval=1000, output_dir="benchmark_results", verbose=True):
    """Run Isolation Forest ASD algorithm on a dataset."""
    ...
    print(f"\n===Running IForestASD on {dataset_name}===")
    ...
    try:
        ...
        # Initialize model
        model = IForestASD(**iforestasd_params)
            ...
            if processed_instances % progress_interval == 0:
                ...
                if verbose:
                    if run_count is not None:
                        print(f"IFASD on {dataset_name} (Run {run_count}): Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)
                    else:
                        print(f"IFASD on {dataset_name}: Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)                
        ...
        results = {
            ...
            "model": "IFASD", # Isolation Forest ASD
            "auc": final_auroc,
        ...
        print(f"IForestASD completed on {dataset_name}")
        ...
        print(f"Error running IForestASD on {dataset_name}: {str(e)}")
        return None

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Isolation Forest ASD on a dataset')
    parser.add_argument('--dataset', type=str, required=True, help='name of the dataset to run IForestASD on')
