...
from pysad_rust import LODA

import warnings
warnings.filterwarnings("ignore")

def run_loda(dataset_name, run_count=None, seed=42, progress_interval=1000, output_dir="benchmark_results", verbose=True):
    """Run LODA algorithm on a dataset."""
    ...
    try:
            ...
            if processed_instances % progress_interval == 0:
                ...
                if verbose:
                    if run_count is not None:
                        print(f"LODA on {dataset_name} (Run {run_count}): Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)
                    else:
                        print(f"LODA on {dataset_name}: Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)                
        ...
        if verbose:
            print(f"LODA completed on {dataset_name}")
        ...
        print(f"Error running LODA on {dataset_name}: {str(e)}")
        return None

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run LODA on a dataset')
    parser.add_argument('--dataset', type=str, required=True, help='name of the dataset to run LODA on')
