...
from pysad_rust import OnlineIsolationForest

import warnings
warnings.filterwarnings("ignore")

def run_oif(dataset_name, run_count=None, seed=42, progress_interval=1000, output_dir="benchmark_results", verbose=True):
    """Run Online Isolation Forest algorithm on a dataset."""
    ...
    try:
        ...
        # Initialize model
        model = OnlineIsolationForest(**oif_params)
            ...
            if processed_instances % progress_interval == 0:
                ...
                if verbose:
                    if run_count is not None:
                        print(f"OIF on {dataset_name} (Run {run_count}): Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)
                    else:
                        print(f"OIF on {dataset_name}: Processed {processed_instances} instances, AUROC: {current_auroc:.4f}, Time: {isinstance_time:.2f} seconds, Memory: {memory_usage:.2f} MB", flush=True)                
        ...
        results = {
            ...
            "model": "OIF", # Online Isolation Forest
            "auc": final_auroc,
        ...
        print(f"OIF completed on {dataset_name}")
        ...
        print(f"Error running OIF on {dataset_name}: {str(e)}")
        return None

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Online Isolation Forest on a dataset')
    parser.add_argument('--dataset', type=str, required=True, help='name of the dataset to run OIF on')
